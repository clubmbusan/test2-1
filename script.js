...
function calculateGroupMode() {
    ...
    // ✅ 배우자가 있는 경우 배우자 관계 공제(5억) 및 추가 공제(최대 30억) 적용
    if (spouse) {
     let spouseInheritanceAmount = (adjustedAssetValue * spouse.sharePercentage) / 100;  // 상속 비용 차감 후 상속 금액 기준
     let spouseRelationshipExemption = 500000000; // 배우자 관계 공제 (5억)

    // ✅ 배우자 추가 공제: 배우자 상속 금액과 30억 중 작은 값을 선택
    let spouseAdditionalExemption = Math.min(spouseInheritanceAmount, 3000000000);

    console.log("📌 배우자 상속 금액:", spouseInheritanceAmount.toLocaleString());
    console.log("📌 배우자 관계 공제 (5억):", spouseRelationshipExemption.toLocaleString());
    console.log("📌 배우자 추가 공제 (최대 30억):", spouseAdditionalExemption.toLocaleString());
...
document.getElementById('result').innerHTML = `
    ...
    ${processedHeirs.map((heir) => `
        ...
        ${heir.relationship !== "spouse" ? `<p>기초 공제: ${Math.round(heir.basicExemption).toLocaleString()} 원</p>` : ""}
